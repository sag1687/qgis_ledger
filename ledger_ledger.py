...
SQLite-based engine that records every modification with granular change tracking:  # noqa: E501
...
class LedgerDB:
    ...
    def _make_path_relative(filepath: str) -> str:
        """Convert absolute path to a relative token to survive cross-OS/Nextcloud moves."""  # noqa: E501
        if not filepath:
            return filepath
    ...
    def _make_path_absolute(filepath: str) -> str:
        """Convert a relative token back to the current machine's absolute path."""  # noqa: E501
        if not filepath:
            return filepath
    ...
    def _insert_commit_record(
        ...
        c.execute(
            ...
            " timestamp, feat_count, commit_type, file_path, parent_id, style_qml) "  # noqa: E501
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (layer_name,
             layer_id,
             user,
    ...
    def create_commit(self, layer: QgsVectorLayer, message: str,
        ...
        commit_id, prev_commit = self._insert_commit_record(layer.name(), layer.id(  # noqa: E501
        ), message, user, layer.featureCount(), "VECTOR", layer.source(), style_str)  # noqa: E501
        c = self._conn.cursor()
    ...
    def create_project_commit(self, message: str, user: str = None) -> int:
        ...
        c = self._conn.cursor()
        c.execute("UPDATE ledger_commits SET file_path=? WHERE id=?",
                  (dest_name, commit_id))
        self._conn.commit()
        return commit_id

    def create_raster_commit(
            self,
            layer,
            message: str,
            user: str = None) -> int:
    ...
    def get_history(self, layer_name: str = None) -> list:
        ...
        base = (
            ...
            "message, timestamp, feat_count, commit_type, file_path, parent_id "  # noqa: E501
            "FROM ledger_commits ")
        if layer_name:
    ...
    def get_commit_info(self, commit_id: int) -> dict:
        ...
        c.execute(
            "SELECT id, layer_name, layer_id, user_name, machine, "
            "message, timestamp, feat_count, commit_type, file_path, parent_id "  # noqa: E501
            "FROM ledger_commits WHERE id=?", (commit_id,), )
        row = c.fetchone()
        if not row:
            return {}
        cols = [
    ...
    def rollback_to(self, layer_or_project, commit_id: int) -> bool:
        """Replace all features in *layer* with the snapshot at *commit_id* or restore project/raster.  # noqa: E501
        ...
        for item in snap:
            ...
            if item["geometry"]:
                geom = QgsGeometry.fromWkt(item["geometry"])
        ...
        layer.addFeatures(feats_to_add)
        ...
        if info.get("style_qml"):
            ...
            # Persist the style to database if supported
            try:
                layer.saveStyleToDatabase("default", "", True, "")
            except Exception:  # nosec B110
                pass

            # Create a .qml sidecar for maximum compatibility
            try:
                src_path = layer.source()
                if "|" in src_path:
                    src_path = src_path.split("|")[0]
                if src_path and os.path.exists(src_path):
                    qml_sidecar = os.path.splitext(src_path)[0] + ".qml"
                    layer.saveNamedStyle(qml_sidecar)
            except Exception:  # nosec B110
                pass
    ...
    def generate_report_data(self, from_date: str = None,
        ...
        for commit in history:
            ...
            modified = sum(
                1 for ch in changes if ch["change_type"] == "MODIFY")
