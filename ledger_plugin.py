...
class _CommitDialog(QDialog):
    """Dialogo custom per inserire msg di commit e scegliere se salvare in Cloud."""
    def __init__(self, title="Nuovo Commit", parent=None):
        ...
        lbl = QLabel("Descrivi le modifiche (Perché):")
        ...
        self.chk_cloud = QCheckBox("Sincronizza subito su Cloud")
        ...
        if LedgerSettings.remote_type() == "webdav":
            ...
            self.chk_cloud.setToolTip("Cloud non configurato nelle Impostazioni")
...
class LedgerPlugin:
    ...
    def _set_status(self, status: str):
        ...
        if status == self.DISCONNECTED:
            self._act_status.setIcon(self._make_icon("⚪", "transparent", "#bdc3c7"))
            self._act_status.setToolTip("QGIS Ledger: Nessun progetto aperto")
        elif status == self.SYNCED:
            self._act_status.setIcon(self._make_icon("🟢", "transparent", "#2ecc71"))
            self._act_status.setToolTip("QGIS Ledger: Sincronizzato con l'ultima versione")
        elif status == self.MODIFIED:
            self._act_status.setIcon(self._make_icon("🟡", "transparent", "#f1c40f"))
            self._act_status.setToolTip("QGIS Ledger: Modifiche in attesa di commit")
        elif status == self.CONFLICT:
            self._act_status.setIcon(self._make_icon("🔴", "transparent", "#e74c3c"))
            self._act_status.setToolTip("QGIS Ledger: Conflitti rilevati!")
    ...
    def initGui(self):
        ...
        self.btn_commit.setPopupMode(QToolButton.InstantPopup)
        self.btn_commit.setToolTip("Salva snapshot (Layer o Progetto)")
        
        self.menu_commit = QMenu(self.btn_commit)
        
        act_commit = QAction(self._make_icon("✔", "#16a085"), "Salva solo questo Layer", self.iface.mainWindow())
        act_commit.triggered.connect(self._on_commit)
        self.menu_commit.addAction(act_commit)
        self._actions.append(act_commit)
        
        act_commit_proj = QAction(self._make_icon("💾", "#8e44ad"), "Salva intero Progetto", self.iface.mainWindow())
        ...
        self._act_status = QAction(self._make_icon("⚪", "transparent", "#bdc3c7"), "Stato Ledger", self.iface.mainWindow())
        self._act_status.setToolTip("QGIS Ledger: Nessun progetto aperto")
    ...
    def unload(self):
        ...
        try:
            if self._sync_timer: self._sync_timer.stop()
            if self._autosave_timer: self._autosave_timer.stop()
        ...
        for p_name in ["timeline_panel", "nextcloud_panel", "main_panel"]:
            try:
                ...
                if p:
                    try: self.iface.removeDockWidget(p)
                    except Exception: pass
                    p.deleteLater()
        ...
        try:
            self.iface.removeCustomActionForLayerType(getattr(self, "upload_nc_action", None))
        except Exception: pass

        try: self.diff_engine.clear_diff()
        except Exception: pass
    ...
    def _connect_and_open_cloud(self, cloud_id: str):
        ...
        missing = False
        if cloud_id == "webdav" and not LedgerSettings.nextcloud_server(): missing = True
        elif cloud_id == "generic_webdav" and not LedgerSettings.webdav_url(): missing = True
        elif cloud_id == "dropbox" and not LedgerSettings.dropbox_token(): missing = True
        elif cloud_id == "onedrive" and not LedgerSettings.onedrive_token(): missing = True
        elif cloud_id == "google_drive" and not LedgerSettings.gdrive_access_token(): missing = True
        
        if missing:
            from qgis.PyQt.QtWidgets import QMessageBox
            QMessageBox.information(
                self.iface.mainWindow(), "QGIS Ledger",
                "Questo servizio Cloud non è ancora configurato.\nApri le Impostazioni per aggiungere le relative credenziali o i token di accesso."
            )
            self._on_settings()
            return
    ...
    def _on_open_project(self):
        ...
        if LedgerSettings.remote_type() != "locale" and self.nextcloud_panel:
            reply = QMessageBox.question(
                ...
                "• 'Yes' per scegliere un file nel disco locale (PC)\n"
                "• 'No' per sfogliare l'archivio CLOUD selezionato",
            ...
            if reply == QMessageBox.No:
                ...
                QMessageBox.information(
                    self.iface.mainWindow(), "QGIS Ledger",
                    "Naviga nel pannello Cloud a destra e fai doppio clic sul file .qgz per scaricarlo e aprirlo."
                )
                return
    ...
    def _on_commit(self):
        ...
        dlg = _CommitDialog("QGIS Ledger — Nuovo Commit", self.iface.mainWindow())
        if not dlg.exec_():
            return
    ...
    def _on_cloud_layer_loaded(self, layer, source_name: str):
        """Called when a cloud vector/raster layer is successfully downloaded and loaded into the project."""
        if not self.ledger.is_connected() or not layer:
            return
            
        # Create an initial commit to track the layer from the start (enables rollback)
        msg = f"Import iniziale da Cloud ({source_name})"
        
        from qgis.core import QgsVectorLayer, QgsRasterLayer
        from functools import partial
        if isinstance(layer, QgsVectorLayer):
            cid = self.ledger.create_commit(layer, msg, LedgerSettings.user_name())
            # Connect editing signals so auto-commit works for this layer
            try:
                layer.editingStarted.connect(partial(self._on_editing_started, layer))
                layer.editingStopped.connect(partial(self._on_editing_stopped, layer))
            except RuntimeError:
                pass
        elif isinstance(layer, QgsRasterLayer):
            cid = self.ledger.create_raster_commit(layer, msg, LedgerSettings.user_name())
    ...
    def _trigger_cloud_sync(self, commit_id: int):
        ...
        if file_path:
            if ctype == "PROJECT":
                ...
                # Upload all ledger project history (the entire .ledger sidecar folder)
                ledger_sidecar = proj_path + ".ledger"
                if os.path.exists(ledger_sidecar) and os.path.isdir(ledger_sidecar):
                    for root, dirs, files in os.walk(ledger_sidecar):
                        for f in files:
                            abs_p = os.path.join(root, f)
                            rel_p = os.path.relpath(abs_p, os.path.dirname(ledger_sidecar))
        ...
        self._act_status.setToolTip("Sincronizzazione Cloud in corso...")
        worker.signals.finished.connect(lambda res: self._act_status.setToolTip("Sincronizzato col Cloud ✅"))
    ...
    def _on_upload_layer_to_nc(self):
        ...
        if ext == ".gpkg":
            # Sorgente già GeoPackage: salviamo lo stile nel DB esistente per auto-embedding
            layer.saveStyleToDatabase("default", "", True, "")
            files_to_sync.append( (src_file, name) )
        else:
            # Non-GPKG (es. shp): converti on-the-fly in GeoPackage per il Cloud
            ...
            result = QgsVectorFileWriter.writeAsVectorFormatV3(
                layer, temp_gpkg, QgsProject.instance().transformContext(), options
            )
            err = result[0] if isinstance(result, tuple) else result
            
            if err == QgsVectorFileWriter.NoError:
                # Carica il gpkg temporaneo per incorporare lo stile QML
                temp_lyr = QgsVectorLayer(f"{temp_gpkg}|layername={layer.name()}", layer.name(), "ogr")
                if temp_lyr.isValid():
                ...
                QMessageBox.warning(self.iface.mainWindow(), "QGIS Ledger", f"Errore durante la conversione in GeoPackage: codice {err}")
                return
        ...
        from qgis.PyQt.QtCore import QThreadPool
        worker = _Worker(sync_task)
        self._act_status.setToolTip("Upload Layer in corso...")
        layer_name = layer.name()
        worker.signals.finished.connect(lambda res: QMessageBox.information(self.iface.mainWindow(), "QGIS Ledger", f"Layer {layer_name} caricato su Cloud con successo!"))
    ...
    def _on_commit_project(self):
        ...
        dlg = _CommitDialog("QGIS Ledger — Commit Progetto", self.iface.mainWindow())
        if not dlg.exec_():
            return
    ...
    def _capture_screenshot(self, commit_id: int):
        ...
        try:
            ...
            if info:
                ...
                ts = info.get("timestamp", "").replace(":", "-").replace(" ", "_").replace("T", "_")
    ...
    def _on_info(self):
        ...
        db_path = self.ledger.db_path()
        db_size_str = "Sconosciuto"
        if db_path and os.path.exists(db_path):
            size_mb = os.path.getsize(db_path) / (1024 * 1024)
            db_size_str = f"{size_mb:.2f} MB"
            
        info_html += (
            f'<tr><td style="padding:4px 12px;color:#bdc3c7;"><b>DB Size:</b></td>'
            f'<td style="padding:4px 12px;color:#ecf0f1;">{db_size_str}</td></tr>'
        )
        ...
        btn_screens = QPushButton("📂 Apri Cartella Screenshot")
