...
class LedgerDiff:
    ...
    def compute_diff(self, commit_id_old: int, commit_id_new: int,
        ...
        snap_old = {
            s["fid"]: s for s in self.ledger.get_snapshot_features(commit_id_old)}  # noqa: E501
        snap_new = {
            s["fid"]: s for s in self.ledger.get_snapshot_features(commit_id_new)}  # noqa: E501
        ...
        for fid in common_fids:
            old = snap_old[fid]
            new = snap_new[fid]
            if old["geometry"] != new["geometry"] or old["attributes"] != new["attributes"]:  # noqa: E501
        ...
        # Create diff layers
        self.ledger.get_commit_info(commit_id_old)
        self.ledger.get_commit_info(commit_id_new)
    ...
    def _create_modified_layers(self, modified_list, suffix, geom_type):
        """Create a single layer for modified features showing their new state."""  # noqa: E501

        # Modified geometry in orange
        uri = f"{geom_type}?crs=EPSG:4326"
        vl_mod = QgsVectorLayer(uri, tr("Modificate") + suffix, "memory")
